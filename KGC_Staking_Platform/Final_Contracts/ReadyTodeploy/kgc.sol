...
contract KGCToken is ERC20, Ownable {
    ...
    bool public secondPhaseOn = false;


    mapping(address => bool) public blackListed;

    event updated(bool _updated);
    event AddressAdd(address _stakingContractAddress);
    ...
    function _transfer(address from, address to, uint256 value) internal virtual override {
        ...
        if(secondPhaseOn){
            ...
            if ( to != owner() && to != stakingContractAddress) {
    ...
    function _AddAddress(address _stakingContractAddress) external onlyOwner {
        
        require(_stakingContractAddress != address(0), "invalid address");
        stakingContractAddress = _stakingContractAddress;

        emit AddressAdd(_stakingContractAddress);
    }
    ...
    function addPairAddress(address _routerAddress, address _usdcAddress, address _stakingContractAddress) external onlyOwner {
        ...
        if (!liquidityAdded && secondPhaseOn){
