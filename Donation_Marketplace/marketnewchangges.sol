...
contract Marketplace is 
    ...
    function initialize(address _mintingContract, address _ownerAddress) initializer public {
        ...
        marketPlaceOwner = payable(_ownerAddress);
    ...
    function startBid( uint256 _listId)  external payable checkSell(_listId)  {
        ...
        require(msg.value > listing[_listId].heighestBid,
            "There is already higer or equal bid exist" );

        uint256 _currentBidAmount = listing[_listId].heighestBid;
        address  _currentBidder = listing[_listId].heighestBidder;
        ...
        listing[_listId].heighestBid = msg.value;
        listing[_listId].heighestBidder = msg.sender;
        listing[_listId].price = msg.value;


        emit Bided(

            listing[_listId].heighestBidder,
    ...
    function endAuction(uint256 _listId) external   {
        ...
        require(!listing[_listId].nftClaimed,"Nft already sold");
        require(!listing[_listId].fixedPrice,"Its on fixedPrice!");
        require(msg.sender == listing[_listId].nftOwner || msg.sender == listing[_listId].heighestBidder,
             "You are not the nftOwner neither the heighest bidder.");
        

        uint256 serviceFee = calulateFee(listing[_listId].heighestBid, serviceFeePercentage);
        uint256 donationFee;
        ...
        uint256 amountSendToSeller = listing[_listId].heighestBid - (serviceFee + donationFee + fiscalFee + royaltyFee);
        ...
        listing[_listId].nftClaimed = true;
        listing[_listId].newOwner = listing[_listId].heighestBidder;
    ...
    function setPlatFormServiceFeePercentage(uint256 _serviceFeePercentage) external onlyOwner{
        require( _serviceFeePercentage > 0  && _serviceFeePercentage < 1600, 
            "fee % must between in 0% to 16% ");
